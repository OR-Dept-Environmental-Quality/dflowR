...
#' 
#' @param x Data frame where col 1 = POSIXct date and col 2 = numeric daily mean flow
...
dflow <- function(x, m, r, yearstart=NA, yearend=NA, wystart="10-01", wyend="09-30") {
  ...
  date99 <- data.frame(date2=format(seq(from=as.POSIXct(paste0(yearstart,"-",wystart), format="%Y-%m-%d"),
                                                  to=as.POSIXct(paste0(yearend,"-",wyend), format="%Y-%m-%d")+(86400*(m-1)),
  ...
  X$jday <- lubridate::yday(as.POSIXct(paste0("1900-", 
                                              lubridate::month(X$date),"-", 
                                              lubridate::day(X$date)), 
                                       format="%Y-%m-%d"))
  
  # define water year
  X$water.year <- ifelse(X$jday >= lubridate::yday(as.POSIXct(paste0(X$year,"-",wystart, format="%Y-%m-%d"))), 
                         X$year + 1, X$year)
  ...
  wystart <- lubridate::yday(as.POSIXct(paste0("1900","-",wystart, format="%Y-%m-%d")))
  wyend <- lubridate::yday(as.POSIXct(paste0("1900","-",wyend, format="%Y-%m-%d")))
